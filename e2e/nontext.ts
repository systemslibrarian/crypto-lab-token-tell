 ...
 * carries a three-token boundary scale — `--border` (#2b3a4d, the decorative
 * divider on panels, readouts, `details` and table rows), `--border-strong`
 * (#3d5169, the `.verdict` box edge) and `--border-control` (#5f7c99, the one
 * meant for controls) — and the control token is applied by a single element
 * rule at `style.css:142-149` covering `button`, `select`,
 * `input[type="text"]`, `input[type="number"]` and `textarea`. The same token
 * is reused for four pieces of non-control chrome (`.act-nav a`,
 * `.provenance-tag`, `.legend .swatch`, `.match`), none of which the CONTROL
 * list below reaches — the nav chips carry no class at all and the other three
 * are `<span>`s. Tokens are discarded by overrides, and that is what this
 * oracle exists to measure rather than trust. Three shapes override or bypass
 * the control token: `button.primary` repaints its border the SAME colour as
 * its own `--accent` fill (`style.css:153`), so it has no edge of its own and
 * lives or dies by fill-vs-surround; `button:hover:not(:disabled)` recolours
 * the edge to `--accent` (`style.css:151`), a state only the drive reaches;
 * and `input[type="range"]` declares `border: none; background: transparent`
 * (`style.css:164`), leaving the appearance to the UA — it never reaches the
 * judged set, and the native-widget exemption below would cover it anyway.
 * `select` goes the other way: `appearance: none` (`style.css:156`) says the
 * author has taken the painting over, so every pixel of it is answerable —
 * including the two `linear-gradient()` arrow sprites inside its own fill.
 *
 * The generated-content half is inert in this repo today — grepped, not
 * assumed: neither `src/style.css` nor `index.html` declares `content` at all,
 * and the page's only generated mark is the `<summary>` disclosure triangle,
 * which is `::marker` (the UA's own, recoloured to `var(--accent)` at
 * `style.css:299`) rather than a `::before`/`::after`. It runs anyway, at every
...
export async function auditNonText(page: Page, within = 'body *'): Promise<NonTextFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     *
     * THIS SAMPLER IS LIVE HERE, and for this oracle it is live ON A CONTROL:
     * `select` paints its dropdown arrow with two `linear-gradient()` layers
     * (`style.css:158-159`), so a `<select>`'s own fill is computed through this
     * path rather than read off `background-color`. The stylesheet's other
     * gradient, the `repeating-linear-gradient` hatch on `.token.masked`
     * (`style.css:245`), sits on a `<span>` and never reaches the control walk;
     * `contrast.ts` is the oracle that judges it. The claim this file used to
     * carry — that the lab declares no gradient — is false of Token Tell.
     *
     * One approximation, stated rather than hidden: `background-position`,
     * `-size` and `-repeat` are not modelled, and `parseStops` reads only `%`
     * stop positions, so `select`'s two 0.35rem corner sprites are evaluated as
     * though they spanned the whole control. A `select` finding from this
     * oracle should be hand-measured before it is acted on.
     */
     ...
     * `<section class="act">` by `src/main.ts`. The control walk itself is
     * cheap (this lab paints a few dozen buttons, selects and textareas), but
     * the generated-content walk below runs over EVERY element under `body`,
     * and Act II's scored token stream alone is one `<span class="token">` per
     * token across a several-hundred-token sample, with two pseudo-element
     * lookups apiece. Without the caches the pass re-reads the same computed
     ...
     * measuring the theme. This page paints a flat
     * `background-color: var(--bg)` LONGHAND on both `html` (`style.css:28-30`)
     * and `body` (`style.css:32-38`) — read out of the stylesheet, not assumed
     * from the template — so the root resolves opaque on the first try and the
     * `<body>` fallback is dormant. Neither declaration is a shorthand
     * `background:` and neither is a gradient, which is exactly what keeps the
     * fallback dormant; both halves stay because either is one refactor away
     * from vanishing. The gradient-aware sampler is borrowed from `contrast.ts`
     * wholesale rather than approximated a second time, and here it is not
     * decoration — `select` really does carry gradient layers in its fill.
     */
    ...
    for (const el of Array.from(document.querySelectorAll(CONTROL))) {
      ...
      // This lab has no tablist, and no control on it currently paints a
      // one-sided edge: every control with a border takes all four sides from
      // the single `--border-control` rule at `style.css:142-149`. The page is
      // full of the one-sided idiom on NON-controls, though — `.thesis` and
      // `.warn-box` widen `border-left` to 5px, `.thesis-strip` is a 3px
      // `border-left` and nothing else, and every `th`/`td` is separated by
      // `border-bottom` alone — so the per-side walk is what keeps "all four
      // sides" an implementation detail of today's stylesheet rather than a
      ...
      // uses exactly that idiom at `style.css:237-245`: `.token` reserves
      // `border-bottom: 3px solid transparent` and each scored state
      // (`.counted-high`, `.counted-low`, `.masked`) fills it in with a real
      // colour, so the token stream never reflows as scores change. Those
      // tokens are `<span>`s and never enter the CONTROL list, so the alpha
      // test has no live subject among this lab's controls today — but it is
      // the test that stops a spacer-bordered control being reported at 1.00:1
      // when it has no fill, no painted edge and is identified by its text
      // alone, which is exactly the case the "is it trying to draw itself as a
